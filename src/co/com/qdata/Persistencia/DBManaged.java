...
public class DBManaged extends SQLiteOpenHelper {
    ...
    @SuppressLint("NewApi")
    public void insertarUrl(int id, String url){
    	SQLiteDatabase db = getWritableDatabase();
    	if(!(db == null)){
    		ContentValues valores = new ContentValues();
    		valores.put("ID", id);
    		valores.put("URL", url);
    		db.insert(getDatabaseName(), null, valores);
    		db.close();
    	}
    }
    ...
    public void modificarUrl(int id, String url){
        ...
        if(!(db == null)){
    		...
    		valores.put("ID", id);
    		valores.put("URL", url);
            db.update(getDatabaseName(), valores, "ID=" + id, null);
            db.close();   
        }
    }
    
    @SuppressLint("NewApi")
    public void borrarUrl(int id) {
        SQLiteDatabase db = getWritableDatabase();
        if(!(db == null)){
            db.delete(getDatabaseName(), "ID="+id, null);
            db.close();          	
        }
    }
    
    public String recuperarURL(String sentencia){
